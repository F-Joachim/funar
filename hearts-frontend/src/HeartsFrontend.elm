...
import HeartsViewModel exposing (..)
...
type alias Model = ViewModel

init : () -> (Model, Cmd Msg)
init =
    \_ -> (emptyViewModel allPlayers, Cmd.none)
...
update msg model =
  case msg of
      ...
      GotGameEvents (Ok gameEvents) ->
          ( List.foldl tableProcessEvent model gameEvents
      ...
      GotGameCommands (Ok commands) ->
          (recordCommands commands model , Cmd.none )
      GotGameCommands (Err err) ->
          Debug.todo (showHttpError err)
      ProcessGameCommands ->
        let (commands, nextModel) = getCommands model
        in (nextModel
            , Cmd.batch (List.map postGameCommand commands))
...
view viewModel =
    let playerHand player =
          Maybe.withDefault emptyHand (Dict.get player.name viewModel.hands)
        playerPile player =
          Maybe.withDefault [] (Dict.get player.name viewModel.piles)
        playerData =
         List.map (\ player ->
        ...
        div [ ]
            ...
            , ul [style "display" "flex", style "flex-direction" "column"]
                 (List.map (\ (player, hand, pile) -> 
                           ...
                           playerData)
            , showGameCommands viewModel.gameCommands
            , showTrick viewModel.trick
            , showWinner viewModel.winner allPlayers ]
